<?php
	$tabellen_spalten_namen = array("Name", "", "Fach 1", "Fach 2", "Fach 3", "Zuletzt eingeloggt", "");
	$tabellen_spalten_breite = array(210, 150, 116, 117, 117, 150, 40);
	...
	if($sort == 10){
		$sql = sprintf("SELECT * FROM experte ORDER BY last_login");
	}
	if($sort == 11){
		$sql = sprintf("SELECT * FROM experte ORDER BY last_login DESC");
	}
	...
	while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
		...
		array_push($tabellen_zeilen_DATA, array($row[name], $row[titel], $row[institut], $row[adresse], $row[region], $row[email], $fach_DATA_ID[$temp_fach[1]],$fach_DATA_ID[$temp_fach[2]],$fach_DATA_ID[$temp_fach[3]], $row[id], $row[last_login]));  
	}
	//
	if($sort == 4){
...
?>
	...
	<tr>
		...
		<td align="center" style="padding-top:4px; padding-bottom:0px; background: #EEE; border-top: 1px solid #000000; <?php if($i2 !=1){ echo "border-left:1px solid #000000;"; } ?> <?php if ($i2==(count($tabellen_spalten_breite)-1) ){ echo "border-right: 1px solid #000000;";} ?>border-spacing:0pt;width: <?php echo $tabellen_spalten_breite[$i2];?>px" valign="top">
			<div >
				<?php if($i2!=1 && $i2 !=6 ){ ?>
				<a style="text-decoration: none;" href="./admin_index.php?rubrik=3&sort=<?php echo $i2*2; ?>" target="_self"><img style="border: 0 none;" src="<?php if($sort ==($i2*2)){echo "assets/sorter_oben_ddd.png"; } else {echo "assets/sorter_oben_aaa.png"; }?>"/></a> 
				<a style="text-decoration: none;" href="./admin_index.php?rubrik=3&sort=<?php echo $i2*2+1; ?>" target="_self"><img style="border: 0 none;" src="<?php if($sort ==($i2*2+1)){echo "assets/sorter_unten_ddd.png"; } else {echo "assets/sorter_unten_aaa.png"; }?>"/></a> 
				<?php } ?>
			</div>
		</td>
		<?php }?>
	...
	<tr>
		...
		<td style="padding:10px; padding-top: 2px; padding-bottom: 0px; border-top:1px solid #000000; <?php if($i2 !=1){ echo "border-left:1px solid #000000;"; } ?> <?php if ($i==($tabellen_zeilen_anzahl-1)){ echo "border-bottom: 1px solid #000000;";} ?> <?php if ($i2==count($tabellen_spalten_breite)-1){ echo "border-right: 1px solid #000000;";} ?> border-spacing:0pt;width: <?php echo $tabellen_spalten_breite[$i2];?>px" valign="top">
			<p>
				...
				<?php   if($i2 == 5){ 
							echo "<div style=\"font-size: 1.0em; text-align:left;\">";
							//echo $tabellen_zeilen_DATA[$i][10];
							date_default_timezone_set("Europe/Zurich");
							if($tabellen_zeilen_DATA[$i][10]){
								echo date("d M Y H:i ", $tabellen_zeilen_DATA[$i][10]);
							}
				...
				<?php 	if($i2 == 6){?> 
				<p>
					<img style="cursor:pointer; border: 0 none;" src="assets/papierkorb.png" onclick="document.forms[0].form_anfrage_aktiv.value= 4; document.forms[0].papierkorb_geklickt.value='<?php echo $tabellen_zeilen_DATA[$i][9]; ?>'; document.forms[0].submit()" /> 
				</p>
				<?php }; ?> 
			</p>
		</td>
