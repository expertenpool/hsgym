...
//NEU:
include("./functions/get_schule_entries.php");
	...
	$sql_zusage = "SELECT * FROM anfrage, zusage, experte WHERE (anfrage.id = zusage.anfrage_id AND experte.id = zusage.experte_id)";
	...
	$sql = "SELECT * FROM anfrage WHERE (sichtbarkeit = '0')";
	...
	while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
		...
		if($schule_DATA_NAME[$row[schule_id]]){
			$tabellen_zeilen_anzahl += 1;
			array_push($tabellen_zeilen_DATA, array($fach_DATA_ID[$row[fach_id]], $art_ID[$row[art]], $row[termin], $row[korrektur], $row[id], $temp_zeit_termin, $temp_zeit_korrektur, $experte_DATA_NAME[$zusage_ID[$row[id]]], $row[klassen_kuerzel], $zusage_ID_DETAIL[$row[id]], $row[schule_id], $schule_DATA_NAME[$row[schule_id]]));  
		}
	...
	//
?>
<?php include("admin/admin_rubrik4_tabelle1.php"); ?>
